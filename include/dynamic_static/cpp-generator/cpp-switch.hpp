...
public:
    ...
    */
    ...
    inline CppSwitch(const Args &... args)
    {
    ...
    TODO : Documentation
    */
    void generate(std::ostream& strm, CppGenerationFlags cppGenerationFlags = 0, std::string_view = { }) const override final;
...
private:
    ...
    inline typename std::enable_if < ArgIndex < sizeof...(Args), void>::type process_ctor_arguments(const std::tuple<const Args&...>& args)
    {
        ...
        if constexpr (std::is_same_v<decltype(arg), CppCase>) {
            cppCases.push_back(arg);
        } else
        if constexpr (std::is_same_v<decltype(arg), CppCase::Collection>) {
            append(cppCases, arg);
        } else
